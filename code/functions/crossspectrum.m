...
strono=8  % Strophe number 7 is shown
...
for i=1:length(Xmattest(1,:,1))-1
    ...
    v1=find(Xmattest(:,i,2)~=0); % find the indecies where this syllable (i) is located (where it is not zero)
    plot(Xmattest(v1,i,2)/Fs,Xmattest(v1,i,1),'g-') % Plot the syllable of interest in green
    if i+1<=length(Xmattest(1,:,1)) % if there is another syllable after the ith one then also plot it
      ...
      text(min(Xmattest(v2,i+1,2)/Fs),0.3,num2str(labelsong(i))) % Add text at y = 0.3 x= start of syllable with values 0 or 1
    ...
    nwin =256;
    [S,F,T] = xspectrogram(Xmattest(:,i,1),Xmattest(:,i+1,1),kaiser(nwin,30),nwin-1,[],Fs,'centered','yaxis');
    figure(2)
